...
write_test(struct ns_entry *ns_entry, kv_value **value)
{
	...
	for(unsigned int i = 0; i < g_num_4Ks; i++) {
		rc = spdk_nvme_ns_cmd_write(ns_entry->ns, ns_entry->qpair, value[i]->value,
				i * lbas_per_4K, /* LBA start */
...
read_test(struct ns_entry *ns_entry, kv_value **value)
{
	...
	for(unsigned int i = 0; i < g_num_4Ks; i++) {
		rc = spdk_nvme_ns_cmd_read(ns_entry->ns, ns_entry->qpair, value[i]->value,
				i * lbas_per_4K, /* LBA start */
				lbas_per_4K, /* number of LBAs */
				io_complete, &sequence, 0);
		if (rc != 0) {
