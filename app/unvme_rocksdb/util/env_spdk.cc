...
public:
	SpdkEnv(const std::string &dir, const std::string &conf,
		const std::string &bdev, uint64_t cache_size_in_mb, int init_from);
...
SpdkEnv::SpdkEnv(const std::string &dir, const std::string &conf,
	...
	opts->init_from = init_from;
	opts->options = const_cast<char*> (mConfig.c_str());
...
void NewSpdkEnv(Env **env, const std::string& dir, const std::string &conf,
		const std::string &bdev, uint64_t cache_size_in_mb, int init_from) {
	SpdkEnv *spdk_env = new SpdkEnv(dir, conf, bdev, cache_size_in_mb, init_from);
