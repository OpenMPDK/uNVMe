...
{
	int i;
	struct json_value** values = (struct json_value**)malloc(sizeof(struct json_value *) * (array->value_cnt + 1));
...
{
	int i;
	struct json_pair **pairs =(struct json_pair**)malloc(sizeof(struct json_pair *) * (obj->pair_cnt + 1));
