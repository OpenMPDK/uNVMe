...
static struct program nvme = {
	...
	.desc = "The '<device>' may be either an NVMe character "\
		...
		"(ex: /dev/nvme0n1) with kernel driver or pci bdf address(ex: 0000:02:00.0) with user level driver",
	.extensions = &builtin,
};
...
static int get_smart_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	const struct argconfig_commandline_options command_line_options[] = {
		...
		{"raw-binary",    'b', "FILE", CFG_STRING,   &cfg.raw_binary,    required_argument, raw},
		{NULL}
	};
	...
	if (!err) {
		if (fmt == BINARY)
			d_raw(cfg.raw_binary, (unsigned char *)&smart_log, sizeof(smart_log));
		else if (fmt == JSON)
...
static int get_error_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		if (!err) {
			if (fmt == BINARY)
				d_raw(cfg.raw_binary, (unsigned char *)err_log, sizeof(err_log));
			else if (fmt == JSON)
...
static int get_fw_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!err) {
		if (fmt == BINARY)
			d_raw(cfg.raw_binary, (unsigned char *)&fw_log, sizeof(fw_log));
		else if (fmt == JSON)
...
static int get_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if(g_kvdd_enabled) {
		return kv_get_log("get-log", desc, argc, argv);
	}
	...
	if (!cfg.log_len) {
		...
		if (!err) {
			if (!cfg.raw_binary) {
				...
				d_raw(cfg.raw_binary, (unsigned char *)log, cfg.log_len);
		} else if (err > 0)
			fprintf(stderr, "NVMe Status:%s(%x)\n",
						nvme_status_to_string(err), err);
		else
...
static int identify_ssd_type(char* bdf)
{
	int err = 0, fd;
	__u32 result;
	...
	#define KV_DELETE_OP_CODE 0xA1
	struct config cfg = {
		.opcode       = KV_DELETE_OP_CODE,
		.flags        = 0,
		.rsvd         = 0,
		.namespace_id = 1,
		.data_len     = 0,
	...
	fd = open_dev((const char *)bdf);
	if (fd < 0)
		return fd;

	err = nvme_passthru(fd, NVME_IOCTL_IO_CMD, cfg.opcode, cfg.flags, cfg.rsvd,
				cfg.namespace_id, cfg.cdw2, cfg.cdw3, cfg.cdw10,
				cfg.cdw11, cfg.cdw12, cfg.cdw13, cfg.cdw14, cfg.cdw15,
				cfg.data_len, NULL, cfg.metadata_len, NULL,
				cfg.timeout, &result);
	if (err < 0)
		perror("passthru");
	else if (err)
...
static void print_list_item(struct list_item list_item)
{
	...
	double nuse = le64_to_cpu(list_item.ns.nuse) / 100.;
	...
	char* ssd_type = "KV";
	if (identify_ssd_type(list_item.node) == 0x1) {
		// if result of kv_retrieve is INVAILD OPCODE
		ssd_type = "LBA";
		nuse = le64_to_cpu(list_item.ns.nuse) * lba;
		u_suffix = suffix_si_get(&nuse);
		sprintf(usage, "%6.2f %2sB / %6.2f %2sB", nuse, u_suffix, nsze, s_suffix);
	} else {
		sprintf(usage, "%6.2f %2s  / %6.2f %2s ", nuse, "%", 100., "%");
	}

	sprintf(format,"%3.0f %2sB + %2d B", (double)lba, l_suffix,
		list_item.ns.lbaf[(list_item.ns.flbas & 0x0f)].ms);
	printf("%-16s %-*.*s %-*.*s %-9d %-26s %-16s %-10s %-.*s\n", list_item.node,
		...
		list_item.nsid, usage, format, ssd_type, (int)sizeof(list_item.ctrl.fr), list_item.ctrl.fr);
}
...
static void print_list_items(struct list_item *list_items, unsigned len)
{
	...
	printf("%-16s %-20s %-40s %-9s %-26s %-16s %-10s %-8s\n",
	    "BDF Address", "SN", "Model", "Namespace", "Usage", "Format", "SSD Type", "FW Rev");
	printf("%-16s %-20s %-40s %-9s %-26s %-16s %-10s %-8s\n",
            "----------------", "--------------------", "----------------------------------------",
            "---------", "--------------------------", "----------------", "----------", "--------");

	// print out list of devices from active spdk process
	for(i = 0; i < g_num_list_device; i++) {
		printf("%s", g_list_device_str[i]);
	}
...
int __id_ctrl(int argc, char **argv, struct command *cmd, struct plugin *plugin, void (*vs)(__u8 *vs, struct json_object *root))
{
	...
	if (!err) {
		if (fmt == BINARY)
			d_raw(cfg.raw_binary, (unsigned char *)&ctrl, sizeof(ctrl));
		else if (fmt == JSON)
...
static int ns_descs(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!err) {
		if (fmt == BINARY)
			d_raw(cfg.raw_binary, (unsigned char *)&nsdescs, 0x1000);
		else if (fmt == JSON)
...
static int id_ns(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!err) {
		if (fmt == BINARY)
			d_raw(cfg.raw_binary, (unsigned char *)&ns, sizeof(ns));
		else if (fmt == JSON)
...
static int get_feature(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!err) {
		if (!cfg.raw_binary || !buf) {
			...
			d_raw(cfg.raw_binary, buf, cfg.data_len);
	} else if (err > 0) {
		fprintf(stderr, "NVMe Status:%s(%x)\n",
				nvme_status_to_string(err), err);
	} else
...
static int format(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	//Configure for KV NVMe SSD Format
	cfg.lbaf = 0;
	cfg.ms = 0;
	cfg.pi = 0;
	cfg.pil = 0;
	cfg.ses = 1;  //Userdata Erase
...
static int resv_report(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		if (fmt == BINARY)
			d_raw(cfg.raw_binary, (unsigned char *)status, cfg.numd << 2);
		else if (fmt == JSON)
...
static int read_cmd(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (g_kvdd_enabled) {
		return kv_submit_io(0, "read", desc, argc, argv);
	} else {
...
static int write_cmd(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	const char *desc = "Copy from provided data buffer (default "\
		...
		"device.";
	if (g_kvdd_enabled) {
		return kv_submit_io(0, "write", desc, argc, argv);
	} else {
		return submit_io(nvme_cmd_write, "write", desc, argc, argv);
	}
}

static int delete_cmd(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	const char *desc = "Delete the given key on the KV-SSD";
	if (g_kvdd_enabled) {
		return kv_submit_io(0, "delete", desc, argc, argv);
	} else {
		fprintf(stderr, "Delete CANNOT be used on LBA SSD(s)");
		return -EINVAL;
	}
}

static int exist_cmd(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	const char *desc = "Check whether a given key-value pair exists or not";
	if (g_kvdd_enabled) {
		return kv_submit_io(0, "exist", desc, argc, argv);
	} else {
		fprintf(stderr, "Exist CANNOT be used on LBA SSD(s)");
		return -EINVAL;
	}
}
...
static int sec_recv(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		if (!cfg.raw_binary) {
			...
			d_raw(cfg.raw_binary, (unsigned char *)sec_buf, cfg.size);
	}
	return err;
}

static int passthru(int argc, char **argv, unsigned int ioctl_cmd, const char *desc, struct command *cmd)
{
	...
	if (err < 0)
		...
		if (!cfg.raw_binary) {
			...
			d_raw(cfg.raw_binary, (unsigned char *)data, cfg.data_len);
	}
	return err;
free_and_return:
	free(data);
...
static int secondary_list(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	char path[264];
	struct dirent **devices;
	struct list_item *list_items;
...
static int list_it(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
	const char *desc = "List all iterators on the NVMe devices";
	if (g_kvdd_enabled) {
		return kv_list_it(0, "list-it", desc, argc, argv);
	} else {
		fprintf(stderr, "Exist CANNOT be used without uNVMe driver");
		return -EINVAL;
	}
}

static int close_it(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
	const char *desc = "List all iterators on the NVMe devices";
	if (g_kvdd_enabled) {
		return kv_close_it(0, "close-it", desc, argc, argv);
	} else {
		fprintf(stderr, "close-it CANNOT be used without uNVMe driver");
		return -EINVAL;
	}
}

static int open_it(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
	const char *desc = "Open a new iterator";
	if (g_kvdd_enabled) {
		return kv_open_it(0, "open-it", desc, argc, argv);
	} else {
		fprintf(stderr, "open-it CANNOT be used without uNVMe driver");
		return -EINVAL;
	}
}

static int read_it(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	const char *desc = "Reads keys or a key/value pair with the given iterator";
	if (g_kvdd_enabled) {
		return kv_read_it(0, "read_it", desc, argc, argv);
	} else {
		fprintf(stderr, "read-it CANNOT be used on LBA SSD(s)");
		return -EINVAL;
	}
}
...
int main(int argc, char **argv)
{
	...
	if (spdk_bypass_cmd(argv[1]) == false) {
		if (kvdd_required_cmd(argv[1]) == true) {
			ret = kvdd_main(argc - 1, &argv[1]);
			if (ret) {
				return ret;
			}
		} else {
