...
int lnvm_do_id_ns(int fd, int nsid, unsigned int flags, const char* raw_binary)
{
	...
	if (!err) {
		if (raw_binary)
			d_raw(raw_binary, (unsigned char *)&nvm_id, sizeof(nvm_id));
		else {
						...
						const char* raw_binary)
{
	...
	if (raw_binary)
		d_raw(raw_binary, (unsigned char *)&bbtbl->blk, bbtblsz);
	else {
...
int lnvm_do_get_bbtbl(int fd, int nsid, int lunid, int chid, unsigned int flags, const char* raw_binary)
{
	struct nvme_nvm_id nvm_id;
	struct ppa_addr ppa;
	int err;
	...
	return __lnvm_do_get_bbtbl(fd, &nvm_id, ppa, flags, raw_binary);
}
